...
from alignment import preprocess_text
...
def load_narc_connlu(narc_conll, built_docs):
    ...
    for file in tqdm(sorted(os.listdir(narc_conll))):
        ...
        if doc_id not in built_docs:
            continue
        data = parse(open(os.path.join(narc_conll, file), "r",
...
def align_tokens(narc_tokens, ud_tokens):
    ...
    while i < len(narc_tokens) and j < len(ud_tokens):
        if narc_tokens[i] == ud_tokens[j]:
            ...
            narc_tokens[i] = preprocess_text(
                narc_tokens[i][len(ud_tokens[j]):])
            j += 1
        else:
            raise NotImplementedError(
                f"This option in token aligning is not implemented: {narc_tokens}, {ud_tokens}")
...
def merge_conllus(narc_conllu, ud_conllu):
    ...
    if ud_conllu[-1]["form"] in [".", "...", ":", ";", "?", "!"]:
        ud_conllu[-1]["misc"] = {k: v for k, v in ud_conllu[-1]
                                 ["misc"].items() if k != "SpaceAfter"}
    return ud_conllu
