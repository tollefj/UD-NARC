...
START_DOC_PATTERN = re.compile(
    ...
    # r'#begin document (.*?)' + '\n')
    # detect whether "newdoc id" is used:
    # e.g. # newdoc id = ap~20050210-762251
    r'#newdoc id = .*' + '\n')
...
def read_file(fpath, sep=None, ignore_double_indices=False,
    ...
    * each being a list of annotation (conll cell).
    ...
    for i, line in enumerate(open(fpath), start=1):
        m = line.startswith("# newdoc")
        # from "# newdoc id = ap~20050210-762251"
        # extract "ap~20050210-762251" as key
        # new document
        if m:
