    ...
    def train(self):
        ...
        for epoch in range(self.epochs_trained, self.config.train_epochs):
            ...
            for doc_id in pbar:
                ...
                if res.span_y:
                    sp1 = self._span_criterion(res.span_scores[:, :, 0], res.span_y[0])
                    s_loss = sp1 / avg_spans
                    # sp2 = self._span_criterion(res.span_scores[:, :, 1], res.span_y[1])
                    # s_loss = (sp1 + sp2) / avg_spans / 2
                else:
